...
def _workspace(batch, hq, hkv, group, dim, capacity, max_context, tile_tokens, match_tile_slots, semantic_pos_ahead):
    ...
    return [
        ...
        *[torch.empty(batch, hkv, device=device, dtype=torch.int32) for _ in range(6)],
        ...
        torch.empty(batch, hkv, max_tiles_context, group, dim, device=device),
        torch.empty(batch, hkv, max_tiles_context, group, device=device),
        torch.empty(batch, hkv, max_tiles_reduce, group, dim, device=device),
        torch.empty(batch, hkv, max_tiles_reduce, group, device=device),
        torch.empty(batch, hkv, max_tiles_tail, group, dim, device=device),
        torch.empty(batch, hkv, max_tiles_tail, group, device=device),
        torch.empty(13, device=device, dtype=torch.int64),
    ]


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_persistent_decode_hit_clean_half_open_cache_row():
    torch.manual_seed(1)
    ...
    # Latest SGLang carries physical cache locations as int64.
    ...
    max_context, tile_tokens, match_tile_slots = 16, 4, 4
    ...
    ext.mac_persistent_decode_bf16(
        ...
        out,
        out_lse,
        *_workspace(batch, hq, hkv, group, dim, capacity, max_context, tile_tokens, match_tile_slots, semantic_pos_ahead),
        max_context,
        tile_tokens,
        match_tile_slots,
        semantic_pos_ahead,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_persistent_decode_miss_multi_request():
    torch.manual_seed(2)
    ext = load_mac_persistent_decode_extension(False)
    device = torch.device("cuda")
    dtype = torch.bfloat16
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_persistent_decode_short_context_writes_empty_cache_row():
    torch.manual_seed(3)
    ext = load_mac_persistent_decode_extension(False)
    device = torch.device("cuda")
    dtype = torch.bfloat16
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_persistent_decode_tie_breaks_by_logical_position_after_wraparound():
    torch.manual_seed(4)
    ext = load_mac_persistent_decode_extension(False)
    device = torch.device("cuda")
    dtype = torch.bfloat16
    ...
    # With past_len=9 and M=4, valid candidates are logical positions [5, 9).
    # Positions 6 and 7 tie exactly; the kernel must choose logical position 6,
    # not the later physical slot.
    ...
    later_prefix_o = torch.randn_like(first_prefix_o)
    later_prefix_lse = torch.randn_like(first_prefix_lse)
    attn_cache[0, first_slot].copy_(first_prefix_o)
    lse_cache[0, first_slot].copy_(first_prefix_lse)
    attn_cache[0, later_slot].copy_(later_prefix_o)
    lse_cache[0, later_slot].copy_(later_prefix_lse)

    max_context, tile_tokens, match_tile_slots = 16, 4, 2
    workspace = _workspace(
        batch, hq, hkv, group, dim, capacity, max_context, tile_tokens, match_tile_slots, semantic_pos_ahead
    )
    out = torch.empty_like(q)
    out_lse = torch.empty(batch, hq, device=device)
    ext.mac_persistent_decode_bf16(
