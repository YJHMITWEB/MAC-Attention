...
template <typename DTypeQ_, typename DTypeKV_, typename DTypeO_, typename IdType_>
struct BatchMACAttentionDecodeParams {
  using DTypeQ = DTypeQ_;
  using DTypeKV = DTypeKV_;
  using DTypeO = DTypeO_;
  using IdType = IdType_;
  ...
  DTypeO* retrieved_o;
  float* retrieved_lse;
  uint32_t* attn_start_pos;
  uint32_t* attn_retrieve_pos;
  int32_t rope_ahead;
  DTypeO* downdate_o;
  float* downdate_lse;
  ...
  __device__ __host__ BatchMACAttentionDecodeParams()
      : q(nullptr),
        q_rope_offset(nullptr),
        paged_kv(),
        matched_q(nullptr),
        retrieved_o(nullptr),
        retrieved_lse(nullptr),
        attn_start_pos(nullptr),
        attn_retrieve_pos(nullptr),
        rope_ahead(256),
        downdate_o(nullptr),
        downdate_lse(nullptr),
        o(nullptr),
        lse(nullptr),
        maybe_alibi_slopes(nullptr),
        padded_batch_size(0),
  ...
  __device__ __host__ BatchMACAttentionDecodeParams(DTypeQ* q, IdType* q_rope_offset,
                                        paged_kv_t<DTypeKV, IdType> paged_kv, 
                                        DTypeQ* matched_q, 
                                        DTypeO* retrieved_o, 
                                        float* retrieved_lse, 
                                        ...
                                        uint32_t* attn_retrieve_pos, 
                                        int32_t rope_ahead,
                                        DTypeO* downdate_o, float* downdate_lse,
                                        DTypeO* o, float* lse,
                                        float* maybe_alibi_slopes, uint32_t num_qo_heads,
                                        IdType q_stride_n, IdType q_stride_h, int32_t window_left,
                                        float logits_soft_cap, float sm_scale, float rope_scale,
      ...
      : q(q),
        ...
        paged_kv(paged_kv),
        matched_q(matched_q),
        retrieved_o(retrieved_o),
        retrieved_lse(retrieved_lse),
        attn_start_pos(attn_start_pos),
        attn_retrieve_pos(attn_retrieve_pos),
        rope_ahead(rope_ahead),
        downdate_o(downdate_o),
        downdate_lse(downdate_lse),
        o(o),
        lse(lse),
        maybe_alibi_slopes(maybe_alibi_slopes),
        padded_batch_size(0),
...
struct MACDecodeParams {
  ...
  IdType* q_rope_offset;
  paged_kv_t<DTypeKV, IdType> paged_kv;
  uint32_t max_running_requests;
  uint32_t cache_capacity;
  DTypeO* attn_cache;
  float* lse_cache;
  uint32_t* hit_indices;
  bool* hit_table;
  uint32_t* cache_req_ids;
  uint32_t* attn_start_pos;
  int32_t downdate_range;
  ...
  IdType* request_indices;
  IdType* kv_tile_indices;
  IdType* kv_head_indices;
  IdType* merge_start_offsets;  // (n * num_qo_heads) start rows per (pos, head)
  IdType* downdate_start_offsets;  // (n,) start rows per (pos, head)
  IdType* o_indptr;
  IdType* kv_chunk_size_ptr;
  bool* block_valid_mask;
  bool partition_kv;

  __device__ __host__ MACDecodeParams()
      : q(nullptr),
        q_rope_offset(nullptr),
        paged_kv(),
        ...
        hit_indices(nullptr),
        hit_table(nullptr),
        cache_req_ids(nullptr),
        attn_start_pos(nullptr),
        downdate_range(0),
        o(nullptr),
        lse(nullptr),
        downdated_o(nullptr),
        downdated_lse(nullptr),
        maybe_alibi_slopes(nullptr),
        padded_batch_size(0),
        num_qo_heads(0),
        q_stride_n(0),
        ...
        rope_rcp_theta(0.0f),
	        request_indices(nullptr),
	        kv_tile_indices(nullptr),
	        kv_head_indices(nullptr),
	        merge_start_offsets(nullptr),
	        downdate_start_offsets(nullptr),
	        o_indptr(nullptr),
	        kv_chunk_size_ptr(nullptr),
	        block_valid_mask(nullptr),
	        partition_kv(false) {}

  __device__ __host__ MACDecodeParams(DTypeQ* q, IdType* q_rope_offset,
                                        paged_kv_t<DTypeKV, IdType> paged_kv,
                                        uint32_t max_running_requests,
                                        uint32_t cache_capacity,
                                        DTypeO* attn_cache,
                                        ...
                                        int32_t downdate_range, 
                                        DTypeO* o, float* lse,
                                        DTypeO* downdated_o, float* downdated_lse,
                                        float* maybe_alibi_slopes, uint32_t num_qo_heads,
                                        IdType q_stride_n, IdType q_stride_h, int32_t window_left,
                                        float logits_soft_cap, float sm_scale, float rope_scale,
                                        float rope_theta)
      : q(q),
        ...
        o(o),
        lse(lse),
        downdated_o(downdated_o),
        downdated_lse(downdated_lse),
        maybe_alibi_slopes(maybe_alibi_slopes),
        padded_batch_size(0),
        num_qo_heads(num_qo_heads),
        q_stride_n(q_stride_n),
...
template <typename DTypeQ_, typename DTypeKV_, typename DTypeO_, typename IdType_>
struct BatchRectificationCacheDecodeParams {
  using DTypeQ = DTypeQ_;
  using DTypeKV = DTypeKV_;
  using DTypeO = DTypeO_;
  using IdType = IdType_;
  ...
  __device__ __host__ BatchRectificationCacheDecodeParams()
      : q(nullptr),
        q_rope_offset(nullptr),
        paged_kv(),
        max_running_requests(0),
        ...
        rope_rcp_theta(0.0f),
		  ...
		  __device__ __host__ BatchRectificationCacheDecodeParams(DTypeQ* q, IdType* q_rope_offset,
	                                        paged_kv_t<DTypeKV, IdType> paged_kv,
	                                        uint32_t max_running_requests,
	                                        uint32_t cache_capacity,
	                                        DTypeO* full_attn,
