...
struct DefaultAttention : AttentionVariantBase {
  ...
  REGISTER_LOGITS_MASK(params, batch_idx, qo_idx, kv_idx, qo_head_idx, kv_head_idx, {
    ...
    // printf("calling masks at /users/PAS2312/yjhmitweb/ai_general/minorTest/flashinfer/include/flashinfer/attention/variants.cuh....\n");
    if constexpr (use_sliding_window) {
...
template <bool use_custom_mask, bool use_sliding_window, bool use_logits_soft_cap, bool use_alibi>
struct MACAttention : AttentionVariantBase {
  static constexpr bool use_softmax = true;
  ...
  // Create closure
  template <typename Params>
  __device__ __host__ MACAttention(const Params& params, uint32_t batch_idx,
                                       uint8_t* smem_ptr) {
    qo_len = params.get_qo_len(batch_idx);
    kv_len = params.get_kv_len(batch_idx);
    if constexpr (use_logits_soft_cap) {
