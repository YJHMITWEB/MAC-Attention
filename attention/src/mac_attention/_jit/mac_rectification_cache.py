...
def _package_root() -> Path:
    # src/mac_attention/_jit/mac_rectification_cache.py -> src/mac_attention
...
def _generated_config_inc(
    *,
    ...
    use_alibi = "true" if pos_encoding_mode == 2 else "false"
...
using AttentionVariant = flashinfer::MACAttention</*use_custom_mask=*/false, USE_SLIDING_WINDOW,
                                                   USE_LOGITS_SOFT_CAP, /*use_alibi_bias=*/{use_alibi}>;
using Params = flashinfer::BatchRectificationCacheDecodeParams<DTypeQ, DTypeKV, DTypeO, IdType>;
"""
...
template cudaError_t BatchRectificationCacheDecodeWithPagedKVCacheDispatched<
    {int(head_dim)}, {posenc}, AttentionVariant, Params>(
    Params params, {dtype_o}* tmp_v, float* tmp_s,
...
def get_mac_rectification_cache_op(
    *,
    q_dtype: torch.dtype,
    kv_dtype: torch.dtype,
    head_dim: int,
    ...
    with FileLock(str(lock_path), thread_local=False):
        ...
        write_text_if_different(
            gen_dir / "mac_rectification_cache_config.inc",
        ...
        write_text_if_different(
            gen_dir / "mac_rectification_cache_kernel_inst.cu",
        ...
        load_cpp_ext(
            name=name,
            sources=[
                str(_vendor_csrc_dir() / "mac_rectification_cache.cu"),
                str(_vendor_csrc_dir() / "mac_rectification_cache_torchlib.cu"),
                str(gen_dir / "mac_rectification_cache_kernel_inst.cu"),
            ],
