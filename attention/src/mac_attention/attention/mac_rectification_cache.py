...
from .._jit.mac_rectification_cache import get_mac_rectification_cache_op
...
class MACRectificationCacheWithPagedKVCacheWrapper:
    def __init__(
        self,
    ...
    def plan(
        ...
        *,
        ...
        use_sliding_window = int(window_left) != -1
        ...
        # Host indptr for planning (reuse pinned buffer to avoid per-call allocations).
        if indptr.device.type == "cpu":
            ...
            # For decode-cache planning we must read indptr on CPU synchronously (the scheduler
            # consumes it immediately). indptr is tiny ([B+1]) so the blocking copy is negligible
            # and avoids D2H race conditions.
            self._indptr_host_buf.copy_(indptr, non_blocking=False)
        ...
        if self._cached_module is None or self._cached_op_key != op_key:
            self._cached_module = get_mac_rectification_cache_op(
                q_dtype=q_dtype,
                kv_dtype=kv_dtype,
...
MACDecodeCacheWithPagedKVCacheWrapper = MACRectificationCacheWithPagedKVCacheWrapper
