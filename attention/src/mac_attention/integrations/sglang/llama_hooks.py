...
class MACForwardBatchState:
    ...
    def prepare(self, forward_batch: Any, device: torch.device, max_running: int, config: Any) -> None:
        ...
        self.ensure(device, max(max_running, batch_size, 1))
        ...
        if req_ids_src.device.type == "cpu":
            ...
            # Persistent readiness is a host-side guard. Copy once per model
            # forward instead of synchronizing in every layer.
...
def _prefill_preserve_q_range(forward_batch: Any, q_len: int) -> tuple[int, int, bool]:
    batch_size = int(getattr(forward_batch, "batch_size", 0))
    plan = build_prefill_suffix_plan(forward_batch, batch_size, q_len)
    if plan is None:
...
class MACMetadata:
    ...
    def init_host_buffer(
        ...
        max_running = int(getattr(server_args, "max_running_requests", batch_size))
        ...
        elif self.mac_cache_ready_host is None or len(self.mac_cache_ready_host) < max_running:
    ...
    def _update_persistent_ready_host(
        ...
        if (
            ...
            idx = torch.tensor(
                sorted(invalidate_reqs),
                dtype=torch.long,
                device=self.device,
            )
            self.mac_cache_ready[idx] = False
            self.mac_cache_epoch[idx] = -1
            self.request_epoch[idx] += 1
    ...
    def prepare_forward_batch(self, forward_batch: Any, config: Any = None) -> None:
        ...
        seq_lens_host = _cpu_int_list(getattr(forward_batch, "seq_lens_cpu", None), batch_size)
...
def llama_attention_forward_around(original_fn: Any, self: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    if not profiling_enabled():
        ...
        cache_ready = (
            ...
            and metadata.unified_query_cache.shape[0] >= max_running
